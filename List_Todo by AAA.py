...
def load_info():
    messagebox.showinfo(
        "Created By", "This is List_TODO V.1.0 \n created by AAA \n Python Mini Project",)
...
root.title("My List_TODO")
root.geometry("270x300")
tasks = []
# tasks = ['tes 1', 'best2', 'dest3']

label_title = tkinter.Label(root, text="List_TODO", bg="light grey")
label_title.grid(row=0, column=0)

label_dsply = tkinter.Label(root, text="", bg="light grey")
label_dsply.grid(row=0, column=1)

label_dsp_count = tkinter.Label(root, text="", bg="light grey")
...
delall_bttn = tkinter.Button(
    root, text="Delete all", bg="white", fg="red", width=15, command=delete_all)
delall_bttn.grid(row=3, column=0)

sort_asc = tkinter.Button(root, text="Sorting (ASC)", bg="White", width=15, command=sort_asc)
sort_asc.grid(row=4, column=0)

sort_dsc = tkinter.Button(root, text="Sorting (DSC)", bg="White", width=15, command=sort_dsc)
...
exit_bttn = tkinter.Button(root, text="Exit app", bg="white", fg="red", width=15, command=exit_app)
exit_bttn.grid(row=10, column=0)

save_button = tkinter.Button(
    root, text="Save TodoList", bg="white", fg="green", width=15, command=save_act)
save_button.grid(row=10, column=1)

load_button = tkinter.Button(
    root, text="Load LastTodolist", bg="white", fg="green", width=15, command=load_act)
load_button.grid(row=8, column=0)

info_button = tkinter.Button(
    root, text="Created By", bg="white", fg="blue", width=15, command=load_info)
